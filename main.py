...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "(ignored when reusing --video-id, since the video keeps whatever "
        "preset it was originally indexed with). 'Advanced' additionally "
        "returns observedPeople, which --require-single-person needs -- "
        "without it, --require-single-person silently keeps every composite "
        "occurrence instead of filtering. Default: Default.",
    )
    parser.add_argument(
        "--min-overlap-seconds",
        type=float,
        ...
        "real co-occurrence). Only affects composite actions like 'person "
        "using phone'; unfiltered by default.",
    )
    parser.add_argument(
        "--merge-gap-seconds",
        type=float,
        default=None,
        help="Collapse occurrences of the same matched term within this many "
