"""
Unit tests for detect_all_actions.py's actual CLI wiring: argument
validation, --check-auth, the --video-id-reuse vs --video-upload
branching, and error handling.

This is a distinct file from tests/test_detect_all_actions.py on purpose:
despite its name, that file only ever exercises `analyze_all`/`report.py`
and never imports the detect_all_actions module itself, so `main()` --
the most complex, least-tested code in the project before this pass --
ran with zero coverage. VideoIndexerClient and Settings.from_env are
replaced with fakes here, the same "no live account needed" approach as
...
def fake_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        detect_all_actions.Settings, "from_env", staticmethod(lambda: FAKE_SETTINGS)
    )
...
def _install_fake_client(
    ...
    monkeypatch.setattr(detect_all_actions, "VideoIndexerClient", lambda settings: fake)
    return fake
...
def test_validate_args_requires_video_or_video_id():
    args = detect_all_actions.build_arg_parser().parse_args([])
    assert detect_all_actions._validate_args(args) is not None
...
def test_validate_args_rejects_missing_video_file(tmp_path: Path):
    ...
    error = detect_all_actions._validate_args(args)
...
def test_validate_args_rejects_out_of_range_min_confidence(bad_value: str):
    ...
    error = detect_all_actions._validate_args(args)
    assert error is not None
    assert "min-confidence" in error
...
def test_validate_args_rejects_out_of_range_min_temporal_iou():
    args = detect_all_actions.build_arg_parser().parse_args(
        ["--video-id", "abc", "--min-temporal-iou", "1.5"]
    )
    error = detect_all_actions._validate_args(args)
    assert error is not None
...
def test_validate_args_accepts_video_id_alone():
    args = detect_all_actions.build_arg_parser().parse_args(["--video-id", "abc"])
    assert detect_all_actions._validate_args(args) is None
...
def test_main_returns_2_for_invalid_args(tmp_path: Path):
    exit_code = detect_all_actions.main(["--video", str(tmp_path / "missing.mp4")])
    assert exit_code == 2
...
def test_main_returns_2_when_config_is_missing(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        detect_all_actions.Settings, "from_env", staticmethod(raise_config_error)
    )

    exit_code = detect_all_actions.main(["--video-id", "abc"])
    assert exit_code == 2


def test_main_check_auth_success(monkeypatch: pytest.MonkeyPatch):
    _install_fake_client(monkeypatch)
    assert detect_all_actions.main(["--check-auth"]) == 0
...
def test_main_check_auth_failure(monkeypatch: pytest.MonkeyPatch):
    ...
    assert detect_all_actions.main(["--check-auth"]) == 1
...
def test_main_reuses_video_id_and_writes_reports(
    ...
    exit_code = detect_all_actions.main(
    ...
    json_path = tmp_path / "abc123.all_actions.json"
    html_path = tmp_path / "abc123.all_actions.html"
    assert json_path.is_file()
    assert html_path.is_file()

    written = json.loads(json_path.read_text(encoding="utf-8"))
    assert written["distinct_action_count"] > 0
...
def test_main_uploads_fresh_video(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    assert (tmp_path / "out" / "clip.all_actions.json").is_file()
...
def test_main_applies_min_confidence_filter(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    ...
    assert exit_code == 0
    written = json.loads((tmp_path / "abc123.all_actions.json").read_text())
    # Every scored fixture confidence (person 0.98, jumping 0.87/0.81/0.76,
    # outdoor 0.95) is below 0.99, so all of those get dropped -- except
    # "trampoline", which has no confidence value at all in the fixture and
    # is kept regardless of threshold (an unscored item can't be compared
    # against one, per analyze_all's documented behavior).
    assert written["occurrence_count"] == 1
    assert {a["name"] for a in written["actions"]} == {"trampoline"}
